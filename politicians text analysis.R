...
is_johnson <- words_joined$proper_name == "Boris Johnson"
...
coefs %>%
  ...
  labs(
    x = NULL,
    title = "Words which have the highest/lowest probability of being spoken by Johnson rather than Corbyn"
  )
...
words_dtm <- speeches %>%
  filter(str_detect(proper_name,c("Corbyn|Ed Miliband|Blair|Boris Johnson|Theresa May|David Cameron|Gordon Brown|Nick Clegg|Vince Cable|Farron|Harriet Harman|John Major|William Hague|Iain Duncan Smith|Michael Howard|Jo Swinson"))) %>% 
  unnest_tokens(word,speech,token = "ngrams", n = 2) %>% select(pp_id,word_count,speech_date,year,proper_name,party,government,word,ministry) %>% 
  mutate(document = paste0(proper_name, speech_date)) %>%
  anti_join(stop_words) %>%
  anti_join(political_words) %>%
  filter(!str_detect(word, "\\d")) %>%
...
words_dtm <- speeches %>%
 filter(str_detect(proper_name,c("Corbyn"))) %>% 
 unnest_tokens(word,speech,token = "ngrams", n = 1) %>% select(pp_id,word_count,speech_date,year,proper_name,party,government,word,ministry) %>% 
 mutate(document = paste0(proper_name, speech_date)) %>%
 anti_join(stop_words) %>%
 anti_join(political_words) %>%
 filter(!str_detect(word, "\\d")) %>%
...
chapters_lda <- LDA(dtm1, k = 5, control = list(seed = 1234))
chapters_lda

chapter_topics <- broom::tidy(chapters_lda, matrix = "beta")
chapter_topics
